...
#define CMD_LISTREC_HTML 40

#define CMD_UNDEFINED 41
...
_cmd_help(const char *cmd, int sockfd) {
    ...
    if( is_master_server ) {
        ...
        ret = matchcmd("^h[\\p{Z}]+(af|ar|a|df|dp|dr|d|h|i|ktf|kt|log|lc|lh|li|lmr|lm|lph|lp|lq|ls|l|n|ot|o|q|rst|rp|sp|st|s|tf|tl|td|t|u|vc|v|wt|x|z|!)$", cmd, &field);
        _writef(sockfd, "ret=%d after command %s",ret,cmd);
    } else {
 ...
 */
static void
_cmd_listrep_html(const char *cmd, int sockfd) {
    char **field = (void *)NULL;
    if (cmd[0] == 'h') {
        _writef(sockfd,
    ...
    // TODO: Plain mail version of repeating recording listing
    // size_t len = strlen(buffer_plain);
    // listrecsbuff(buffer_plain+len, maxlen-len, n, 3);

    char title[255];
    snprintf(title,255,"List of upcoming repeating recordings");
...
cmdinit(void) {
    ...
    cmdtable[CMD_LISTREC_HTML]      = _cmd_listrep_html;
}
